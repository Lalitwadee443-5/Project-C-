...
namespace PayrollSytem
{
    public partial class frm_Employees : Form
    {
        ...
        private void btnempsave_Click(object sender, EventArgs e)
        {
            ...
            if (txtcode.Text == "" || txtfname.Text== "" || txtlname.Text == "" || txtnname.Text == "" 
                || txtaddress.Text == "" || txtcontact.Text == "" || txtstatus.Text == "" || txtidc.Text == "" 
            ...
            else
            {
                ...
                sql = "INSERT INTO `employee` (`emp_code`, `emp_fname`, `emp_lname`, `emp_nname`" 
                   + ", `address`, `contact`, `status`, `birth_date`, `id_card`, `emp_sex`, `emp_age`"
                   + ", `emerg_contct`) VALUES ('" + txtcode.Text + "','" + txtfname.Text + "','" + txtlname.Text
                   + "','" + txtnname.Text + "','" + txtaddress.Text + "'," + txtcontact.Text + ",'" + txtstatus.Text
                   + "','" + dtpdbirth.Text + "','" + txtidc.Text + "','" + rdo + "'," + txtage.Text + ",'" + txtcontact.Text + "')";
        ...
        private void txtcode_TextChanged(object sender, EventArgs e)
        {
            ...
            if(config.dt.Rows.Count > 0)
            {
                ...
                txtnname.Text = config.dt.Rows[0].Field<string>("emp_nname").ToString();
                ...
                txtidc.Text = config.dt.Rows[0].Field<string>("id_card").ToString();
                ...
                txtcontact.Text = config.dt.Rows[0].Field<string>("emerg_contct").ToString();
...

        {
            ...
            else
            {
                ...
                + "', `emp_lname`='" + txtlname.Text + "', `emp_nname`='" + txtnname.Text 
                + "', `address`='" + txtaddress.Text + "', `contact`='" + txtcontact.Text + "', `status`='" + txtstatus.Text 
                + "', `birth_date`='" + dtpdbirth.Text + "', `id_card`='" + txtidc.Text + "', `emp_sex`='" + rdo 
        ...
        private void list_Employee()
        {
            sql = "SELECT `emp_code` AS 'CODE',CONCAT( `emp_fname`,' ', `emp_lname`) AS 'NAME'" 
             + ", `emp_age` AS 'AGE', `emp_sex` AS 'GENDER', `id_card` AS 'ID_CARD', `address` AS 'ADDRESS'" 
